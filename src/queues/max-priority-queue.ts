 ...
 */
export class MaxPriorityQueue<T extends defined> {
	public readonly array = new Array<HeapEntry<T>>();
	public length = 0;

	public static readonly instanceof = <T extends defined>(value: unknown): value is MaxPriorityQueue<T> =>
		typeIs(value, "table") && getmetatable(value) === MaxPriorityQueue;
...
const metatable = MaxPriorityQueue as LuaMetatable<MaxPriorityQueue<never>>;
// hello luau users
metatable.__len = (maxPriorityQueue) => maxPriorityQueue.length;
metatable.__tostring = (maxPriorityQueue) =>
	`MaxPriorityQueue<[\n${maxPriorityQueue.array.map(({ priority, value }) => `\t{ priority: ${priority}, value: ${value} },`).join("\n")}\n]>`;
